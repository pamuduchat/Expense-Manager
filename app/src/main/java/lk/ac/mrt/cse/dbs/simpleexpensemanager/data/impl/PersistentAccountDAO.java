...
public class PersistentAccountDAO implements AccountDAO {
    ...
    public Account getAccount(String accountNum) throws InvalidAccountException {
        ...
        Cursor cursor = db.query(
                TABLE_ACCOUNT,   // process the query
                new String[]{COL_ACCNO, COL_BANKNAME, COL_ACCHOLDERNAME, COL_BALANCE},
                selection,
    ...
    public List<Account> getAccountsList() {
        ...
        Cursor cursor = db.query(
                TABLE_ACCOUNT,   // generate quary to get the columns
                new String[]{COL_ACCNO, COL_BANKNAME, COL_ACCHOLDERNAME, COL_BALANCE},
                null,
                null,
                null,
    ...
    public void addAccount(Account account) {
        ...
        rowValues.put(COL_ACCNO,account.getAccountNo());
        rowValues.put(COL_BANKNAME,account.getBankName());
        rowValues.put(COL_ACCHOLDERNAME,account.getAccountHolderName());
        rowValues.put(COL_BALANCE,account.getBalance());
        //inserting
        db.insert(TABLE_ACCOUNT,null,rowValues);
        db.close();
    }
    ...
    public void updateBalance(String accountNo, ExpenseType expenseType, double amount) throws InvalidAccountException {
         ...
         db = dataBaseHandler.getWritableDatabase();
         String selection = COL_ACCNO + " = ?";
         Cursor cursor = db.query(  //process the query
                 TABLE_ACCOUNT,
        ...
        double balance;
         ...
         if(expenseType == ExpenseType.INCOME){
             rowValues.put(COL_BALANCE,balance+amount);
         }
         else{
             rowValues.put(COL_BALANCE, balance - amount);
         }
         db.update(TABLE_ACCOUNT,rowValues, COL_ACCNO + " =?", new String[] {accountNo});
         cursor.close();
         db.close();
